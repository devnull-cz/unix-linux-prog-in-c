...
\begin{slide}
\sltitle{creating new process group/session}
...
\begin{slide}
\sltitle{Vytvoøení procesu: \texttt{fork()}}
\begin{center}
...
\begin{itemize}
\item \label{FORK} dítì je témìø pøesnou kopií rodièe. Základní atributy, které
se li¹í, jsou:
\begin{itemize}
\item \emsl{PID} procesu a \emsl{parent PID}
\item pokud mìl rodiè více vláken, má syn pouze to, které \texttt{fork}
zavolalo; o tom více na stranì \pageref{FORKALL}, a¾ se dostaneme k vláknùm.
Tyto informace by ka¾dopádnì mìly být v manuálové stránce \texttt{fork(2)}.
\item úètovací èasy se nastaví na 0
\item nedìdí se nastavení \texttt{alarm} a zámky souborù
\end{itemize}
\item co je dùle¾ité je, ¾e tabulky deskriptorù jsou stejné v obou procesech,
èím¾ více procesù mù¾e sdílet a posunovat spoleènou pozici v souboru, a také ¾e
masky signálù se nemìní, viz strana \pageref{SIGNALBLOCKINGEXAMPLE}.
\item pro urychlení a men¹í spotøebu pamìti se adresový prostor nekopíruje, ale
pou¾ívá se mechanizmus \emph{copy-on-write}.
\item je logické, ¾e pøi úspì¹ném provedení volání otec dostane jako návratovou
hodnotu volání \texttt{fork} PID syna a syn èíslo 0; syn si toti¾ mù¾e velmi
jedno\-du\-¹e svùj vlastní PID zjistit voláním \texttt{getpid}. Otec ale neumí
jednodu¹e zjistit PID syna, navíc v situaci, pokud ji¾ døíve vytvoøil syny jiné.
\item pøíklad: \priklad{fork/fork.c}
\item \label{VFORK} zajímavostí mù¾e být volání \texttt{vfork}, kterým se kdysi
obcházel problém, kdy draze vytvoøený proces byl okam¾itì pøepsán následným
voláním \texttt{exec}. Tento problém byl dávno vyøe¹en ji¾ zmínìným pou¾itím
mechanizmu copy-on-write, ale pøí\-klad \priklad{fork/vfork.c} ukazuje, jak toto
volání fungovalo.
\end{itemize}
...
\begin{slide}
\sltitle{Spu¹tìní programu: \texttt{exec}}
\texttt{extern char **\funnm{environ};\\
int \funnm{execl}(const char *\emph{path}, const char *\emph{arg0}, ... );}
\begin{itemize}
\item spustí program definovaný celou cestou \texttt{path}, dal¹í argumenty se
pak pøedají programu v parametrech \texttt{argc} a \texttt{argv} funkce
\texttt{main}. Seznam argumentù je ukonèen pomocí \texttt{(char *)0}, tj.
\texttt{NULL}. \texttt{arg0} by mìl obsahovat jméno programu (tj. ne celou
cestu)
\item úspì¹né volání \texttt{execl} se nikdy nevrátí, proto¾e spu¹tìný program
zcela nahradí dosavadní adresový prostor procesu. 
\item program dìdí promìnné prostøedí, tj. obsah \texttt{environ}. 
\item handlery signálù se nahradí implicitní obsluhou
\item zavøou se deskriptory souborù, které mají nastavený pøíznak
\texttt{FD\_CLOEXEC} (implicitnì není nastaven).
\end{itemize}
\end{slide}
...
\begin{itemize}
\item o signálech více na stranì \pageref{SIGNALS}
\item v \texttt{path} musí být celá (absolutní nebo relativní) cesta ke
spustitelnému souboru. Obsah promìnné prostøedí \texttt{PATH} se pou¾ívá jen
pøi voláních \texttt{execlp} a \texttt{execvp}, kdy¾ argument
\texttt{path} neobsahuje znak \texttt{'/'}.
\item nìkdy se pou¾ívá \texttt{argv[0]} rùzné od jména spustitelného souboru.
Napø. \texttt{login} vlo¾í na zaèátek jména spou¹tìného shellu znak
\texttt{'-'}. Shell podle toho pozná, ¾e má fungovat jako login-shell, tj.
napøíklad naèíst \texttt{/etc/profile}.
\item \texttt{exec} nepøedá kontrolu naètenému programu v pamìti pøímo, ale
pøes dynamický linker (té¾ nazývaný \emph{loader}), poté co ho (= toho
\emph{loader}a) namapuje do adresového prostoru procesu. Loader následnì
namapuje potøebné dynamické objekty a teprve poté pøedá kontrolu aplikaci. Viz
také strana \pageref{RUNTIMELINKER}. Pro jednoduché ovìøení staèí vytvoøit
program obsahující pouze tøeba volání \texttt{open}. Tento program pak
spus»te pomocí \texttt{truss(1)} takto: \texttt{truss ./a.out}. Uvidíte, která
volání se pou¾ijí je¹tì pøedtím, ne¾ se zavolá \texttt{open}.
\item \texttt{exec} nezmìní hodnoty RUID a RGID. A pokud je to program s
nastaveným SUID bitem, tak se EUID a uschované UID nastaví na UID majitele
spustitelného souboru.
\item dne¹ní unixové systémy umí spou¹tìt i skripty, které zaèínají øádkem\\
...
\begin{slide}
\sltitle{Varianty slu¾by \texttt{exec}}
\setlength{\baselineskip}{0.8\baselineskip}
\texttt{int \funnm{execv}(const char *\emph{path}, char *const \emph{argv}[]);} 
\begin{itemize}
\item obdoba \texttt{execl()}, ale argumenty jsou v poli \texttt{argv},
jeho¾ poslední prvek je \texttt{NULL}. 
\end{itemize}
...
\end{minipage}
\begin{itemize}
\item obdoba \texttt{execl()}, ale místo \texttt{environ} se pou¾ije
\texttt{envp}. 
\end{itemize}
\begin{minipage}{\slidewidth}
\vspace{-1ex}\texttt{\begin{tabbing}
...
\end{minipage}
\begin{itemize}
\item obdoba \texttt{execv()}, ale místo \texttt{environ} se pou¾ije
\texttt{envp}. 
\end{itemize}
...
\begin{itemize}
\item obdoby \texttt{execl()} a \texttt{execv()}, ale pro hledání
spustitelného souboru se pou¾ije promìnná \texttt{PATH}.
\end{itemize}
\end{slide}

\begin{itemize}
\item \emsl{l} = list (tj. lineární seznam argumentù), \emsl{v} = vector (tj.
pole ukazatelù na øetìzce), \emsl{e} = environment (tj. pøedávají se promìnné
prostøedí), \emsl{p} = PATH.
\item kromì \texttt{execlp} a \texttt{execvp} je nutné jméno programu v¾dy
zadávat celou cestu.
\item v¹echny varianty kromì \texttt{execle} a \texttt{execve}
pøedávají spou¹tìnému programu také své aktuální prostøedí, tj. obsah
pole \texttt{environ}.
\item z mnì neznámých historických dùvodù neexistuje volání s
\emsl{p} a \emsl{e} dohromady.
\item \label{EXEC_DATE} pøíklad: \priklad{exec/exec-date.c}
\item \label{EXECL} následující pou¾ití volání \texttt{execl} je ¹patnì, proto¾e
mu chybí povinný parametr pro \texttt{argv[0]}:

\begin{verbatim}
	execl("/bin/ls", NULL);
\end{verbatim}

Na nìkterých systémech to má zajímavý efekt. Jeliko¾ je \texttt{NULL} brán jako
oèekávaný \texttt{argv[0]}, dal¹í data na zásobníku jsou akceptována jako
ukazatele na øetìzce, dokud se nenalezne dal¹í \texttt{NULL}. V na¹em pøípadì
tak pøíkaz \texttt{ls} zkou¹í zobrazit informace o souborech, jako jejich¾ jména
jsou pou¾ity øetìzce nastavení promìnných prostøedí z pole \texttt{environ}, o
nich¾ ji¾ víme, ¾e se voláním \texttt{execl} spou¹tìnému programu pøedávají:
