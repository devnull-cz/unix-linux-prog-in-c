...
\item \emsl{Common Object File Format (COFF)} -- AT\&T System V, superceeded
\emsl{a.out}
...
use ELF, there are other widely used systems that do not.  For example, Mac OS X
(which is a certified UNIX system) uses the \emph{Mach-O} format, short for
\emph{Mach Object} file format.  Each Mach-O file is made up of one Mach-O
header, followed by a series of load commands, followed by one or more segments,
each of which contains between 0 and 255 sections.
