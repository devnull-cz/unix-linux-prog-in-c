...
Pøíklad: \example{pmap/proc-addr-space.c}
...
a \texttt{MAP\_SHARED}.  There is also \texttt{MAP\_FIXED}, with which the
...
viz pøíklad \example{mmap/lseek.c}.  Signál dostanu stejnì tak v situaci, kdy
...
Neplatí to v¹ude úplnì pøesnì, viz pøíklad \example{mmap/sigbus.c}.
\item Pøi pou¾ití \texttt{MAP\_FIXED} namapování souboru nahradí pøípadné
pøedchozí mapování stránek v rozsahu.
\texttt{addr} a¾ \texttt{addr+l{}en-1}, viz pøíklad \example{mmap/override.c}.
...
signál \texttt{SIGBUS}, viz pøíklad \example{mmap/aligned.c}.
\end{itemize}
