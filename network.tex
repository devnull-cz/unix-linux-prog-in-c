...
\pageref{FCNTL}).  Repetitious \texttt{read} (which is non-blocking, see page
\pageref{OPEN}) is then called on all descriptors.  Sleeping in every loop is
needed in order not to hammer the kernel with syscalls.  For every invocation,
you either read some data or get \texttt{-1} with \texttt{errno} set to
\texttt{EAGAIN}.  However, the disadvantages are: active waiting, frequent
...
\texttt{select} and then call \texttt{read} only for those descriptors
marked as \emph{ready}.
...
see page \pageref{ACCEPT}, so it is necessart to set it.
