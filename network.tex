...
\begin{slide}
\sltitle{Convert hostnam to addresses: \texttt{getaddrinfo()}}
...
\texttt{NULL} (wildcard socket). Simiarly for the member \texttt{ai\_flags} of
the \texttt{addrinfo} structure used for the \emph{hint} parameter -- for server
it is usually set to \texttt{AI\_PASSIVE}.
...
writing or which experienced exceptional state. If none such descriptor is
found, waits till \texttt{timeout} expires.
(\texttt{NULL} \dots means arbitrarily long wait). The \texttt{nfds} parameter
sets the range of file descriptor values (\texttt{0,~...,~\emsl{nfds-1}}).
...
(or set the flag after \texttt{open} with \texttt{fncnl} -- with second
parameter \texttt{O\_SETFL}, not \texttt{O\_SETFD}, see page \pageref{FCNTL}).
Non-blocking \texttt{read} is then called on all descriptors (sleeping in
between is needed in order not to hammer the kernel with syscalls).
The disadvantages are: active waiting, frequent user-kernel mode switching,
possible delay (up to the wait period between the iterations of the cycle).
Correct solution of this situation is to use e.g. \texttt{select} and then
call \texttt{read} only for those descriptors, which \texttt{select} marked as
\emph{ready}.
\item \label{BUSY_WAITING_C} busy waiting example:
\example{select/busy-wait.c}. Notice that newly created socket does not have the
\texttt{O\_NONBLOCK} flag, see page \pageref{ACCEPT}, so it is necessart to set
it.
\item \emph{Ready} means that \texttt{read} or \texttt{write} with nullified
\texttt{O\_NONBLOCK} flag would not block, therefore there has to be some data
ready (\texttt{read} can e.g. return 0 for end-of-file or -1 for error).
...
header). It does not mean that there was an error on given descriptor.
The error is determined by checking \texttt{errno} after given syscall returns
-1.
...
specified event happened.
\emsl{It is therefore necessary to refresh the sets before next call to
\texttt{select}.}
Typically the sets are implemented as bit masks however it does not have to be
so. The programmer does not have to deal with that. Traversing the sets is
only possible one bit at a time and using \texttt{FD\_ISSET}.
\item \texttt{select} is handy also for the possibility to write into pipe or
socket -- waiting for the other side to consume part of the buffer and thus free
space for more incoming data.
\item It is possible to use NULL instead of descriptor set. Special case is to
use NULL for all the sets in which case the syscall will block until the timeout
expires or a signal is delivered.
...
\item It is possible to use \texttt{FD\_SETSIZE} for the \emsl{nfds} parameter
...
\label{NON_BLOCKING_CONNECT} Example: \example{tcp/non-blocking-connect.c}.
...
\item On Solaris \texttt{poll} is system call, \texttt{select} then
library called implemented as \texttt{poll} wrapper.
