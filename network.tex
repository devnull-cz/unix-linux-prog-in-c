...
For the demonstration it is necessary to establish at least one connection,
...
\texttt{getaddrinfo}.
\item \hlabel{GETADDRINFO} example: \example{resolving/getaddrinfo.c}
...
\item \hlabel{GETNAMEINFO} example: \example{resolving/getnameinfo.c}
...
\emph{ready}.
\item \hlabel{BUSY_WAITING_C} Busy waiting example: \example{select/busy-wait.c}.
...
\item Example: \hlabel{SELECT_C} \example{select/select.c}
...
\hlabel{NON_BLOCKING_CONNECT} Example: \example{select/non-blocking-connect.c}.
