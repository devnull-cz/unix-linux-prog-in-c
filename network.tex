...
a special value \texttt{INADDR\_ANY}, denoting the connection will be accepted
on any IP address on any interface of the host.
...
created.
\end{itemize}
\item A client does not have to call \funnm{bind}() in which case the kernel
will assign one of available ports and the primary IP address on the interface
through which the target can be accessed.  In general, as a client, you do not
need a specific outgoing port so \funnm{bind}() is usually not needed.  However,
some legacy services, eg.  \texttt{rsh}, require the client to connect from a
privileged port (0-1023).  Such a client must call \funnm{bind}() to use such a
port and also have enough privileges to do that.
\end{itemize}
...
constants.
\item There are other socket types, see the \texttt{socket(2)} man page.  For
example, \texttt{SOCK\_RAW} for full protocol access or to work with a routing
table.  In order to use \texttt{SOCK\_RAW} you usually need additional
privileges.  That is a reason why a command \texttt{ping}, which works with ICMP
headers of sent packets, might need an SUID privilege:
...
bajtù.} Poøadí bajtù bylo vysvìtleno na stranì \pageref{BYTE_ORDERING}, dal¹í
...
\example{tcp/up-to-listen-only.c}.
...
\item \label{TCP_SINK_SERVER_C} Pøíklad: \example{tcp/tcp-sink-server.c}
\end{itemize}
...
roury.
\item Pøíklad: \label{CONNECT_C} \example{tcp/connect.c}
...
\item pøíklad: \label{UDP_SERVER_C} \example{udp/udp-server.c}
\end{itemize}
...
\item Pøíklad: \label{UDP_CLIENT_C} \example{udp/udp-client.c}.
\end{itemize}
...
reset zpùsobí chybu \texttt{ECONNRESET}. Pøíklad \example{tcp/linger.c}.
\end{itemize}
...
Pøíklad: \example{resolving/addresses.c}.
\end{itemize}
