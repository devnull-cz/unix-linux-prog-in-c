...
\begin{slide}
\sltitle{Waitin for connection: \texttt{listen()}}
...
\begin{itemize}
\item The system can change \emph{backlog} if it is not in a supported range.
...
\begin{itemize}
\item The ``remote end'' would be a socket on which a \funnm{connect}() is
called on a Unix system, or it could be possibly something else on other
...
\item For connection-less services (UDP), \funnm{connect}() may be used as well.
However, it will only set the remote address so that \funnm{send}() and
\funnm{recv}() which do not have a remote address parameter can be used.
