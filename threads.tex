...
\emph{privilege separation} and is not applicable in classical threaded
environment.
\end{itemize}
...
\item how to avoid all threads to be blocked when one threads blocks
e.g.~on~I/O
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item creates a new thread, puts its ID to \emph{\texttt{thread}}
\item with attributes from \emph{\texttt{attr}}, e.g. its stack size,
...
pthread\_t tid;
int i;
extern void *start\_routine(void *);

for (i = 0; i < N; i++)
    pthread\_create(&tid, NULL, start\_routine, &i);
\end{alltt}
...
for (i = 0; i < N; i++)
    pthread\_create(&tid, NULL, start\_routine, (void *)(intptr\_t)i);
\end{alltt}
...
\begin{slide}
\sltitle{Exiting and waiting for a thread to exit}
ifdef([[[NOSPELLCHECK]]], [[[
...
arbitrary thread to finish}.  The rationale is that since there is not parent--child
relation, it was not deemed necessary.  However, some systems provide such
...
\pageref{PTHREAD_CLEANUP}.  For example, if cancelling a thread that is holding
a lock, a cleanup function can be used to unlock it.
...
\texttt{(void *)} type. The \emph{\texttt{destructor}} function is called for
all keys whose value is not \texttt{NULL} on thread termination.
\end{itemize}
ifdef([[[NOSPELLCHECK]]], [[[
...
\begin{itemize}
\item binds pointer \emph{\texttt{value}} to previously created
\emph{\texttt{key}}
\end{itemize}
ifdef([[[NOSPELLCHECK]]], [[[
...
\begin{itemize}
\item returns the value of \emph{\texttt{key}}
\end{itemize}
\end{slide}
...
\item Destructors are called at thread exit in unspecified order on non-deleted
keys with a value different from \texttt{NULL}. Its value is set to \texttt{NULL}
...
that open and close a code block. This enforces particular usage pattern
...
the child), such deadlocks will be avoided. That is because when locking mutexes
...
% \cline{2-4}
\end{tabular}}
\item Static mutex initialization using the before mentioned macro will set default
