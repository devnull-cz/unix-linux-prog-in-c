...
\item with introduction of threads, a process becomes just data -- address
...
\item Even that debugguers typically support threads, the problem is that
debugging changes timing so when you start debugging things that run in
parallel, the problem may not reproduce anymore.  That is usually not a problem
...
\emsl{Multithreaded Programming Guide} available on
\url{http://docs.oracle.com}.
\item \label{PRIVILEGE_SEPARATION} An example situation when you do not want to
use threads is if you want to change real and effective UID of processes.  Take
...
a \texttt{longjmp}. Pøíklad: \example{pthreads/setjmp.c}.
\end{itemize}
...
Pøíklad: \example{mutexes/not-my-lock.c}.
...
\item \label{MUTEX_RACE} pøíklady: \example{mutexes/race.c} a
