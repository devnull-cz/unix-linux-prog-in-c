...
\item function \emph{start\_fun}() will be started in the thread using argument
\emph{arg}.  After the function returns, the thread ceizes to exist.
...
increase.}  It is usually around 64-512 kilobytes so if you use an automatic
array of 1~MB, it is very possible that you end up with a core dump.
If you want a stack of a greater size than what is the system default, you have
to use \emph{attr} when creating a thread.
...
\item The contents of the stack the finishing thread is undefined so you should
use pointers to the function local variables anywhere in memory pointer to by
\emph{val\_ptr}.
...
\item Functions \funnm{pthread\_setcancelstate}()
\funnm{pthread\_setcanceltype}() provides similar functionality to threads as is
...
original value is used as a parameter to the destructor.  pøedána v parametru.
If, after all the destructors have been called for all non-\texttt{NULL} values
with associated destructors, there are still some non-\texttt{NULL} values with
...
\item \label{FORKALL} Note that who \funnm{fork}() works also depends on the
system used.  For example, Solaris before version 10 (ie. before 2005)
...
\example{pthreads/fork-not-in-main.c}, \example{pthreads/forkall.c}
...
kritickou sekci), na které se èeká v handleru. Toto samozøejmì pøedpokládá, ¾e
zamykání a odemykání v handlerech dodr¾uje zamykací pravidla ("protokol")
stanovený pro celý program a nedojde tedy k deadlocku.
