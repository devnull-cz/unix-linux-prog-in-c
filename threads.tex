...
\begin{itemize}
\item Each thread has a fixed size thread which \emsl{does not automatically
increase.}  It is usually around 64-512 kilobytes so if you use an automatic
array of 256 KB, it is possible that you end up with a core dump.
If you want a stack of a greate size than it is the system default, you have to
use \emph{attr} when creating a thread.
Example: \example{pthreads/pthread-stack-overflow.c}
\item You can read more about thread specific data is on page
