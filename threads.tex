...
in such environment. It may or work, or not.
...
Solarisu 10, Solarisu 11, FreeBSD 7.2 and a Linux distribution, that reports
...
\begin{itemize}
\item for architectures where the addition operation is not atomic
...
\item some systems supply functions for atomic operations,
(e.g. \texttt{atomic\_add(3c)} in Solaris); generally it is better
to use the routines from C11 standard via \emph{stdatomic.h}.
\item There is API for miscellaneous types and operations, e.g. addition:
\end{itemize}
\begin{verbatim}
    #include <stdatomic.h>
...
\emsl{x}, into which it adds the numbers from 1 to specified value \emsl{arg}.
...
On Solarisu it is possible to observe this with:
\begin{verbatim}
