...
\item The most common operations with \texttt{lseek} are three: seeting the
...
which is not by itself a disasterous thing, and makes it more difficult to find
it:
...
and \texttt{errno} is set.  \texttt{NULL} is also returnd upon reaching the end
of the directory but in that case \texttt{errno} is not changed.
...
However, it is not part of SUSv4 and thus not portable, unfortunatelly.  The
portable way is to call \texttt{stat} on each directory entry.
