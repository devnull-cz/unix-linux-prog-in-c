...
GitHub:\\
\url{https://github.com/devnull-cz/unix-prog-in-c}
...
\item For the API, we will follow \emph{Single UNIX Specification, version~4}
(SUSv4). Systems that submit to the Open Group for certification and pass
conformance tests are termed to be compliant with a UNIX standard UNIX~V7.
Versions of Solaris, AIX, HP-UX a macOS on various architectures are
compliant with the the previous version SUSv3
(\url{http://www.opengroup.org/openbrand/register/xy.htm}).
\item The specific source code examples are usually tested on macOS and Linux.
\end{itemize}
...
\begin{slide}
\sltitle{Current UNIX and Unix-like Systems}
Main proprietary Unix operating systems:
\begin{itemize}
...
\item SCO: \emsl{SCO Unix} (defunct)
\item Xinuos (formerly Novell): \emsl{UNIXware}
\end{itemize}
Open source:
\begin{itemize}
\item \emsl{FreeBSD}, \emsl{NetBSD}, \emsl{OpenBSD}, \emsl{DragonflyBSD}
\item many \emsl{Linux} distributions
\end{itemize}
\end{slide}
...
\item Note that Linux is a kernel, not the whole system.  In contrast to
...
\item language variants
    ...
    \end{itemize}
\item success of C overcame the success of Unix itself
\end{itemize}
\end{slide}
...
standard came to existence as ANSI X3.159-1989 "Programming Language C", and is
mostly known as ``ANSI C'', or C89, and the command line name for the compiler
itself was \texttt{c89}.
...
and informally called C99.  After that, there were three technical corrigendums,
...
\item byte ordering -- depends on the architecture
    ...
    \end{tabular}}
    \end{itemize}
\item in Unix, a text file line ends with a single character \emsl{LF} (not two
characters \emsl{CRLF} as on Windows). Calling \verb.putc('\n'). thus prints
only one character.
...
\emph{return}, is a character 0x0D (13).  To further confuse the enemy,
"classic" Mac OS used a single \emsl{CR} as line breaks.  As present time
macOS comes from the Unix world, it also uses \emsl{LF} now.
...
\item pøíklad (viz také \example{err/err.c}):
