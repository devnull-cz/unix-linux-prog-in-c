...
\pdfbookmark[0]{synchronizace a komunikace procesù}{synchro}
\begin{slide}
\sltitle{Obsah}
\begin{itemize}
...
\item signály
\item \emsl{synchronizace a komunikace procesù}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Synchronizace procesù a IPC

changequote([[[, ]]])

\pdfbookmark[1]{konflikt pøi sdílení dat}{conflict}
...
zdroj.
\item \label{RACE_C} pøíklad: \priklad{race/race.c}
\end{itemize}
...
\pdfbookmark[1]{vzájemné vylouèení}{mutexcl}
...
\pdfbookmark[1]{synchronizaèní mechanismy}{synchromechs}
...
\texttt{O\_EXCL}
\item pøíklad: \priklad{file-locking/lock-unlock.c}, a pou¾ijte spoleènì se
shellovým skriptem \priklad{file-locking/run.sh}.
...
\item \label{FCNTL_LOCKING} pøíklad: \priklad{file-locking/fcntl-locking.c}
\item \label{FCNTL_FIXED_RACE_C} pøíklad na fcntl (je to pomocí fcntl
,,opravená'' verze døívej¹ího pøí\-kla\-du \priklad{race/race.c} ze strany
...
\texttt{fcntl}.
\item Pøíklad \priklad{file-locking/lockf.c} demonstruje jak funguje
